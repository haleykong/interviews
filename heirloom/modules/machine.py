...
class Machine:
    ...
    def _actuate(self):
        ...
        """
        # KY-ANH - Is this the right way to implement the configuration?
        if self.configs.simulate.machine_faults:
            if np.random.binomial(1, self.MACHINE_FAULT_RATE, 1):
                return ErrorCodes.ACTUATE.FAULTED
            else:
                return ErrorCodes.ACTUATE.SUCCESS
        else:
            return ErrorCodes.ACTUATE.NO_SIMULATION
    ...
    def _read_rfid(self):
        ...
        if self.configs.simulate.machine_faults:
            ...
            return ErrorCodes.RFID.NO_SIMULATION
    ...
    def carry_op(self, operation, vstack, retry=False):
        ...
        if error_codes[0] == ErrorCodes.CHECK.SUCCESS:
            # Actuation
            error_codes += [self._actuate()]
            # RFID read
            error_codes += [self._read_rfid()]
        else:
            error_codes += [None] * (NUM_OP_CHECK - 1)

        # KY-ANH - Where should this updating go? Is there a better way to do
        # this?
        ...
        if error_code_sum == 0:
            # Update for Pick or Retry Pick
            # Remove item from vstack and add it to the machine
            if operation[0] == Actions.PICK:
                slot_index = operation[1]
                self.cur_item = vstack.slots[slot_index]
                vstack.remove_item(slot_index)
            elif (operation[0] == Actions.RETRY and
                  self.transactions[-1].operation[0] == Actions.PICK):
                slot_index = self.transactions[-1].operation[1]
                self.cur_item = vstack.slots[slot_index]
                vstack.remove_item(slot_index)
            # Update for Place or Retry Place
            # Add item to vstack and remove it from the machine
            elif (operation[0] == Actions.PLACE):
                slot_index = operation[1]
                vstack.add_item(slot_index, self.cur_item)
                self.cur_item = None
            elif (operation[0] == Actions.RETRY and
                  self.transactions[-1].operation[0] == Actions.PLACE):
                slot_index = self.transactions[-1].operation[1]
                vstack.add_item(slot_index, self.cur_item)
                self.cur_item = None
    ...
    def retry(self, vstack):
        ...
        if err_code != ErrorCodes.RETRY_CHECK.RETRIES_EXCEEDED:
            ...
            metadata = {
                'operation': (
                    Actions.RETRY,
                    self.transactions[-1].metadata['operation'][1]),
...
class Transaction:
    # KY-ANH - check this implementation
    """Class that holds the transaction metadata
    """
    # KY-ANH - Should this be an absolute path?
    FORMAT_FILE = "/Users/haleykong/interviews/heirloom/data/configs/txn_configs/base.yml"
    ...
    # KY-ANH - This currently only prints "Transaction Summary" if I log
    # machine.transactions[x]. If I do machine.transaction.metadata, it still
    # pops up with the "Configuration Summary" since metadata is a dict.
    # What can I do?
    def __repr__(self):
        repstr = '-' * 79 + '\nTransaction Summary\n'
